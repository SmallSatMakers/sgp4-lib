...
public:
    ...
    {
        m_encoded = TimeSpan(
                        static_cast<int64_t>(AbsoluteDays(year, doy) * TicksPerDay))
                        .Ticks();
    }
    ...
    int AbsoluteDays(int year, int month, int day) const
    {
         ...
         * + plus prior years divisible by 400 days
         */
        int result = DayOfYear(year, month, day) - 1 + 365 * previousYear + previousYear / 4 - previousYear / 100 + previousYear / 400;
